...
Usage:
    site-audit.py                       # audits ./public, writes docs/audit/<date>.md
    site-audit.py --check               # same, but exit 1 if any ERROR found
    site-audit.py --public public --out -   # print report to stdout
"""
...
SITE_SUFFIX_RE = re.compile(r"\s*[·|–-]\s*Sanket Sudake\s*$")
...
def bare_title(title: str) -> str:
    """Title without the ' · Sanket Sudake' site suffix, for length checks."""
...
def main():
    ...
    ap.add_argument("--out", default=None, help="report path, or '-' for stdout (default: docs/audit/<date>.md)")
    ...
    if args.out == "-":
        ...
        out = Path(args.out) if args.out else Path("docs/audit") / f"{today}.md"
