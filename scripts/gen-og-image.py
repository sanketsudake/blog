...
def compose(fields, kind, background, fonts):
    img = background.copy()
...
def site_default(fonts):
    ...
    draw.text((MARGIN, 350), "Distributed Systems · Infrastructure · AI Infra",
              font=sfont, fill=MUTED)
    bfont = load_font(fonts["bold"], FONT_CANDIDATES_BOLD, 30)
...
def discover(root: Path) -> list[Path]:
    ...
    for p in sorted((root / "content" / "posts").glob("*/index.md")):
        targets.append(p)
    talks = root / "content" / "talks"
    for p in sorted(talks.glob("*.md")):
        if p.name != "_index.md":
            targets.append(p)
    for p in sorted(talks.glob("*/index.md")):
        targets.append(p)
    return targets
...
def main():
    ...
    if args.default or args.all_content:
        ...
        out.parent.mkdir(parents=True, exist_ok=True)
        site_default(fonts).save(out, "PNG")
    ...
    for md in targets:
        ...
        generate_for(md, root, fonts, bg, out_override)
