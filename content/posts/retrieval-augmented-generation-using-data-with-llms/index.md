...
![High-level RAG](high-level-rag.webp)
*(High-level RAG)*
...
We use [langchain](https://python.langchain.com/docs/get_started/introduction) and [OpenAI text generation](https://platform.openai.com/docs/guides/text-generation) APIs for implementation, but most of the parts explained in the below pipelines are customizable. You can refer to the source from [SkillScout GitHub Repo](https://github.com/infracloudio/skillscout/tree/main).
Eg. LLM or Embeddings model could be swapped with any compatible implementation, as you can notice from [the core package](https://github.com/infracloudio/skillscout/tree/main/core).
...
![Document Preprocessing Pipeline](document-preprocessing-pipeline.webp)
*(Document Preprocessing Pipeline)*
...
![Vector Store Ingestion Pipeline](vector-store-ingestion-pipeline.webp)
*(Vector Store Ingestion Pipeline)*
...
![Retrieval Augmented Generation Pipeline](retrieval-augmented-generation-pipeline.webp)
*(Retrieval Augmented Generation Pipeline)*
...
Now in Query 2, **_"his"_** is a possessive pronoun which refers to John Doe.  We may rewrite Query 2 as "What's John Doe's total years of experience?"
...
We load our minimal Resume data as mentioned in [SkillScout project README](https://github.com/infracloudio/skillscout).  After generating a summary of resumes and loading data in the vector store, we can see the chat interface via the app command.
...
We hope you found this post informative and engaging. We'd love to hear your thoughts on this post, so do start a conversation on [LinkedIn](https://www.linkedin.com/in/sanketsudake/) :)
