...
A **scheduling cycle** runs one pod at a time through:
...
An asynchronous **binding cycle** — PreBind, Bind, PostBind — follows in a goroutine so the next pod's scheduling cycle starts immediately.
Default behaviors (resource fit, taints, affinity, spreading) are themselves plugins.

**Queues.**
Pods awaiting scheduling live in three structures:
...
With the `GenericWorkload` gate on (beta in v1.37, off by default) a whole PodGroup is one queue entity.
...
**Answer.**
Scheduling cycle, serial per pod:
